...
@property (nonatomic, strong) UIBarButtonItem *pickerViewBarButtonItem1;
...
+(void)dismissWithCompletion1:(BOOL)completion
{
    [[self sharedView] setPickerHidden1:completion];
}
-(void)dismiss{
...
-(void)setPickerHidden1: (BOOL)hidden
{
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  CGFloat iOSVersion1 = [[[UIDevice currentDevice] systemVersion] floatValue];
  ...
  _pickerViewBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(dismiss)];
    _pickerViewBarButtonItem1 = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(dismiss1)];

    
  _pickerViewToolBar.items = @[_pickerViewBarButtonItem1,flexibleSpace, _pickerViewBarButtonItem];
